import edu.princeton.cs.algs4.{Point2D, RectHV}
import org.scalatest.matchers.should.Matchers
import org.scalatest.wordspec.AnyWordSpec

import java.lang.{Double => JavaDouble}
import scala.io.Source
import scala.jdk.CollectionConverters.IterableHasAsScala

class KdTreeSpec extends AnyWordSpec with Matchers {

  "KDTree implementation" should {
    "handle api correctly with one point in the square " in {
      val kdTree = new KdTree()
      kdTree.isEmpty shouldBe true

      val p = new Point2D(0.1, 0.2)
      kdTree.insert(p)
      kdTree.isEmpty shouldBe false
      kdTree.size() shouldBe 1
      kdTree.contains(p) shouldBe true
      kdTree.nearest(new Point2D(0.9, 0.9)) shouldBe p

      val containedRange = kdTree.range(new RectHV(0.1, 0.1, 0.2, 0.2)).asScala.toList
      containedRange shouldBe List(p)

      val emptyRange = kdTree.range(new RectHV(0.2, 0.2, 0.3, 0.3)).asScala.toList
      emptyRange shouldBe List()
    }

    "throw IllegalArgumentException when null range is given" in {
      val kdTree = new KdTree()
      kdTree.isEmpty shouldBe true

      val p = new Point2D(0.1, 0.2)
      kdTree.insert(p)
      kdTree.isEmpty shouldBe false
      kdTree.size() shouldBe 1
      kdTree.contains(p) shouldBe true
      kdTree.nearest(new Point2D(0.9, 0.9)) shouldBe p

      intercept[IllegalArgumentException] {
        kdTree.range(null).asScala.toList
      }
    }

    "handle api correctly with two points in the square " in {
      val kdTree = new KdTree()
      kdTree.isEmpty shouldBe true

      val p1 = new Point2D(0.1, 0.2)
      val p2 = new Point2D(0.9, 0.9)
      kdTree.insert(p1)
      kdTree.isEmpty shouldBe false
      kdTree.contains(p1) shouldBe true
      kdTree.contains(p2) shouldBe false
      kdTree.size() shouldBe 1
      kdTree.nearest(new Point2D(0.7, 0.8)) shouldBe p1

      kdTree.insert(p2)
      kdTree.isEmpty shouldBe false
      kdTree.size() shouldBe 2
      kdTree.contains(p2) shouldBe true
      kdTree.nearest(new Point2D(0.7, 0.8)) shouldBe p2

      val containedOneElementInRange = kdTree.range(new RectHV(0.1, 0.1, 0.2, 0.2)).asScala.toList
      containedOneElementInRange shouldBe List(p1)

      val containedTwoElementInRange = kdTree.range(new RectHV(0.0, 0.0, 0.99, 0.99)).asScala.toList
      containedTwoElementInRange shouldBe List(p1, p2)

      val emptyRange = kdTree.range(new RectHV(0.2, 0.2, 0.3, 0.3)).asScala.toList
      emptyRange shouldBe List()
    }


    "handle API correctly for input10.txt" in {
      val points = readPointsFromResource("kdtree/input10.txt")
      val kdTree = new KdTree()
      kdTree.isEmpty shouldBe true
      points.foreach(point => kdTree.insert(point))
      kdTree.isEmpty shouldBe false
      kdTree.size() shouldBe points.size

      val (smallestX, smallestY) = smallestCoordinates(points)
      val (largestX, largestY) = largestCoordinates(points)
      // lack of better name
      val smallPoint = new Point2D(smallestX, smallestY)
      val largePoint = new Point2D(largestX, largestY)
      val distinctPoints = points.toSet


      kdTree.nearest(smallPoint) shouldBe smallPoint
      kdTree.nearest(largePoint) shouldBe largePoint

      val rectangle = rectangleWithWholeArea(points)
      val contained = kdTree.range(rectangle).asScala.toList
      contained.size shouldBe distinctPoints.size
    }

    "handle API correctly for input100K.txt" in {
      val points = readPointsFromResource("kdtree/input100K.txt")
      val distinctPoints = points.toSet
      val kdTree = new KdTree()
      kdTree.isEmpty shouldBe true
      points.foreach(point => kdTree.insert(point))
      kdTree.isEmpty shouldBe false
      kdTree.size() shouldBe distinctPoints.size

      val (smallestX, smallestY) = smallestCoordinates(points)
      val (largestX, largestY) = largestCoordinates(points)
      // lack of better name
      val smallPoint = new Point2D(smallestX, smallestY)
      val largePoint = new Point2D(largestX, largestY)


      kdTree.nearest(smallPoint) shouldBe smallPoint
      kdTree.nearest(largePoint) shouldBe largePoint

      val rectangle = rectangleWithWholeArea(points)
      val contained = kdTree.range(rectangle).asScala.toList
      contained.size shouldBe distinctPoints.size
    }

    "handle API correctly for input10K.txt" in {
      val points = readPointsFromResource("kdtree/vertical7.txt")
      val distinctPoints = points.toSet
      val kdTree = new KdTree()
      kdTree.isEmpty shouldBe true
      points.foreach(point => kdTree.insert(point))
      kdTree.isEmpty shouldBe false
      kdTree.size() shouldBe distinctPoints.size

      val rectangle = rectangleWithWholeArea(points)
      val contained = kdTree.range(rectangle).asScala.toList
      contained.size shouldBe distinctPoints.size
    }

    "handle API correctly for all input files" in {
      val files = List(
        "circle10.txt",
        "circle100.txt",
        "circle1000.txt",
        "circle10000.txt",
        "circle4.txt",
        "horizontal8.txt",
        "input5.txt",
        "input10.txt",
        "input100K.txt",
        "input10K.txt",
        "input1M.txt",
        "input200K.txt",
        "input20K.txt",
        "input400K.txt",
        "input40K.txt",
        "input800K.txt",
        "input80K.txt",
        "vertical7.txt")
      files.foreach { name =>
        println(s"Executing brute-force point set test for $name")
        val points = readPointsFromResource(s"kdtree/$name")
        val (smallestX, smallestY) = smallestCoordinates(points)
        val (largestX, largestY) = largestCoordinates(points)
        // lack of better name
        val smallPoint = new Point2D(smallestX, smallestY)
        val largePoint = new Point2D(largestX, largestY)
        val distinctPoints = points.toSet

        val kdTree = new KdTree()
        kdTree.isEmpty shouldBe true
        // before inserting points nearest point should be null
        kdTree.nearest(smallPoint) shouldBe null

        // inserting all the points
        points.foreach { point =>
          kdTree.insert(point)
          kdTree.contains(new Point2D(point.x(), point.y())) shouldBe true
          kdTree.nearest(new Point2D(point.x(), point.y())) shouldBe new Point2D(point.x(), point.y())
        }

        kdTree.isEmpty shouldBe false
        kdTree.size() shouldBe distinctPoints.size

        kdTree.nearest(smallPoint) shouldBe smallPoint
        kdTree.nearest(largePoint) shouldBe largePoint

        val rectangle = rectangleWithWholeArea(points)
        val contained = kdTree.range(rectangle).asScala.toList
        contained.size shouldBe distinctPoints.size
      }
    }
  }

  private def smallestCoordinates(points: List[Point2D]): (Double, Double) = {
    val point = points.minBy(_.x())
    (point.x(), point.y())
  }

  private def largestCoordinates(points: List[Point2D]): (Double, Double) = {
    val point = points.maxBy(_.x())
    (point.x(), point.y())
  }

  private def rectangleWithWholeArea(points: List[Point2D]): RectHV = {
    new RectHV(0.0, 0.0, 1.0, 1.0)
  }

  private def readPointsFromResource(fileName: String): List[Point2D] = {
    val inputStream = getClass.getResourceAsStream(fileName)
    Source.fromInputStream(inputStream).getLines().filter(_.trim.nonEmpty).map { line =>
      val tokens = line.split(" ")
      val x = JavaDouble.parseDouble(tokens.head)
      val y = JavaDouble.parseDouble(tokens.last)
      new Point2D(x, y)
    }.toList
  }
}
